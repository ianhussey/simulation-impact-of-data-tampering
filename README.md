...
|---|---|---|---|
| **Aims** | Characterise a tampering procedure: tamper count to reach *p* < .05, proportion never significant within the cap, effect-size bias, and comparison across strategies | as *t*-test | as *t*-test, minus the bias measure; adds prevalence as a factor |
...
| **Estimands** | `tampers_needed` (median, IQR, 95% PI, and as a proportion of N); empirical detection rate within the tampering budget and its complement, the proportion never significant; final-dataset Hedges' *g* — which, since the truth is 0, **is** the bias | as *t*-test, with Pearson *r* (and Fisher's *z*) as the bias | `tampers_needed` only (no standardised effect-size bias in the binary setting) |
...
Held constant everywhere: the true null; α = .05; a **tampering budget of 50% of the sample** (`max_*_proportion = 0.5`); and the greedy *most-helpful-first* rule — each step alters the single data point that moves the test statistic furthest toward significance, then the test is re-run.
...
Two quantities in every table and plot must not be confused. The value in parentheses is **±1 MCSE**: how precisely the simulation has pinned down that summary. The bracketed interval is the **spread of the simulated studies themselves** — either the interquartile range (`iqr_lower`/`iqr_upper`) or the **95% prediction interval** (`pi_lower`/`pi_upper`, the 2.5th and 97.5th percentiles), i.e. the range within which a single new study's result would fall 95% of the time. Plots show the 95% PI, truncated at the tampering budget. The 0th and 100th percentiles (`min`/`max`) are computed and kept in the summary objects but are never tabled or plotted: the maximum of *K* draws drifts upward as *K* grows, so it is not a stable quantity.

**Censoring, and why the median is the headline statistic.** `tampers_needed` is right-censored: an iteration that exhausts the 50% budget without reaching *p* < .05 has a count of *at least* the budget, with no upper bound. The median, IQR and prediction interval are therefore computed over **all** iterations with the censored ones ranked above the budget, so any percentile below the censoring point is estimated exactly and any percentile above it is reported as `> budget` rather than being silently replaced by the survivors' percentile. Summarising only the iterations that reached significance answers a different question — "*given* that tampering worked, how many edits?" — and biases the answer downwards; it shifts the median by 1–4 tampers in 24 of the 135 condition cells, and in two cells (chi-square at N = 20 with prevalence .05, where 57–60% never reach significance) it reports a small number where the honest answer is that the median is not attainable within the budget.

The **mean cannot be computed that way at all**, since the censored counts are unbounded above. The `mean_reached` / `sd_reached` columns are therefore conditional on reaching significance, are downward-biased for the unconditional mean by construction, and are reported as a within-condition skew diagnostic against the median rather than as a headline summary — they must not be compared across conditions with different censoring fractions. The one respect in which the mean is better behaved: its MCSE is smooth and always positive, whereas the bootstrap MCSE of a median of integer counts is frequently exactly 0 at *K* = 10,000 (the median is pinned) and is `NA` when the median itself is censored.
...
Every strategy is iterative and greedy: generate null data → test → while non-significant and under the cap, apply one operation targeting the case(s) most opposed to the desired result → re-test.
...
- **Single threshold and budget.** All results are conditioned on α = .05 and a 50% tampering cap, both reported transparently.
