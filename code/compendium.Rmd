...
if(file.exists("../data/simulation_condition_changing.rds")){
  ...
  alter_data <- function(data, 
    ...
    smallest_intervention <- data |>
      filter(condition == "intervention") |>
      arrange(score) |>
      slice_head(n = n_participants_changed)
    ...
    # identify the N largest absolute scores from the selected control + intervention groups
    largest_absolute_scores <- smallest_intervention |>
      bind_rows(largest_control) |>
    ...
    data <- data |>
      mutate(
        condition = case_when(
          rowname %in% largest_absolute_scores & condition == "intervention" ~ "control",
          rowname %in% largest_control & condition == "control" ~ "intervention",
          TRUE ~ condition
          ),
        altered = case_when(
          rowname %in% c(largest_absolute_scores) ~ TRUE,
          TRUE ~ altered
          )
        )
...
simulation_condition_changing_summary <- simulation_condition_changing |>
  ...
  rename(p_altered_data = p) |>
  # summarize across iterations
  group_by(n_per_condition, n_participants_changed) |>
...
data_summary_ordered <- simulation_condition_changing_summary |>
  ...
  # order the rows within the columns in the way you want them to appear left to right
  arrange(desc(n_per_condition), n_participants_changed) |>
  mutate(rank = row_number()) |>
  ...
  # select only the columns that you had multiple values for in the expand.grid() call in the simulation as well as `outcome` and `rank`
  # rename them if necessary to plot nicely
  mutate(n_participants_changed = as.factor(n_participants_changed),
         n_per_condition = as.factor(n_per_condition)) |>
  dplyr::select(`N changed` = n_participants_changed, 
