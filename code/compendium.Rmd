...
if(file.exists("../data/simulation_unwarranted_data_exclusion.rds")){
  
  simulation_unwarranted_data_exclusion <- read_rds("../data/simulation_unwarranted_data_exclusion.rds")
  ...
  write_rds(simulation_unwarranted_data_exclusion, "../data/simulation_unwarranted_data_exclusion.rds", compress = "gz")
...
data_summary_ordered <- simulation_unwarranted_data_exclusion_summary |>
  ...
  arrange(desc(n_per_condition), n_participants_dropped_per_condition) |>
  ...
  mutate(n_participants_dropped_per_condition = as.factor(n_participants_dropped_per_condition),
         n_per_condition = as.factor(n_per_condition)) |>
  dplyr::select(`N participants` = n_per_condition,
                `N dropped` = n_participants_dropped_per_condition, 
                outcome,
                rank) 
...
ggsave(filename = "../communications/plots/exclusions.pdf",
       plot = p_multiverse,
       device = "pdf",
       width = 6,
       height = 5)
...
## Condition swapping (pairs of participants)

Involves swapping the conditions of pairs of participants (maintaining balance)

```{r fig.height=25, fig.width=7}

if(file.exists("../data/simulation_condition_swapping.rds")){
  
  simulation_condition_swapping <- read_rds("../data/simulation_condition_swapping.rds")
  ...
  write_rds(simulation_condition_swapping, "../data/simulation_condition_swapping.rds", compress = "gz")
...
ggplot(simulation_condition_swapping$altered_data[[iteration]], aes(condition, score, color = altered)) +
  geom_point(position = position_jitter(width = 0.1)) +
  scale_color_viridis_d(begin = 0.3, end = 0.7) +
  theme_linedraw()

bind_cols(
  simulation_condition_swapping$analysis_results_original_data[[iteration]] |>
    rename(p_original = p),
  simulation_condition_swapping$analysis_results_altered_data[[iteration]] |>
    rename(p_altered = p),
) |>
  mutate_if(is.numeric, janitor::round_half_up, digits = 3) |>
  kable() |>
...
# summarise simulation results over the iterations ----
simulation_condition_swapping_summary <- simulation_condition_swapping |>
  # convert `analysis_results` nested-data-frame column to regular columns in the df. in this case, the p value.
  unnest(analysis_results_original_data) |>
  rename(p_original_data = p) |>
  unnest(analysis_results_altered_data) |>
...
simulation_condition_swapping_summary |>
  mutate_if(is.numeric, janitor::round_half_up, digits = 2) |>
  kable() |>
  kable_classic(full_width = FALSE)
...
data_summary_ordered <- simulation_condition_swapping_summary |>
  # the outcome variable must be called `outcome`
  # eg the estimand of your simulation, whether the false positive rate, estimated effect size, etc.
  # replace `positive_rate` with
  mutate(outcome = altered_data_false_positive_rate) |>
  ...
  # order the rows within the columns in the way you want them to appear left to right
  arrange(desc(n_per_condition), n_participants_swapped_per_condition) |>
  mutate(rank = row_number()) |>
  ...
  mutate(n_participants_swapped_per_condition = as.factor(n_participants_swapped_per_condition),
         n_per_condition = as.factor(n_per_condition)) |>
  dplyr::select(`N participants` = n_per_condition,
                `N swapped` = n_participants_swapped_per_condition, 
                outcome,
                rank) 
...
ggsave(filename = "../communications/plots/swapped.pdf",
       plot = p_multiverse,
       device = "pdf",
       width = 6,
       height = 5)
...
## Condition changing (individual participants)

Involves changing the condition of a single participant (disribupting balance, but might be more ecologically valid)

```{r fig.height=25, fig.width=7}

if(file.exists("../data/simulation_condition_changing.rds")){
  
  simulation_condition_changing <- read_rds("../data/simulation_condition_changing.rds")
  ...
  alter_data <- function(data, 
                         n_participants_changed_in_control,
                         n_participants_changed_in_intervention) {
    ...
    smallest_intervention <- data |>
      filter(condition == "intervention") |>
      arrange(score) |>
      slice_head(n = n_participants_changed_in_intervention) |>
      pull(rowname)
    ...
    largest_control <- data |>
      filter(condition == "control") |>
      arrange(desc(score)) |>
      slice_head(n = n_participants_changed_in_control) |>
      pull(rowname)
  ...
  experiment_parameters_grid <- expand_grid(
    ...
    sd_control = 1,
    sd_intervention = 1,
    n_participants_changed_in_control = c(0:3),
    n_participants_changed_in_intervention = c(0:3),
    iteration = 1:500 # data are quite cumbersome due to many permutations; set to 500 for now
  ) |>
    filter(!(n_participants_changed_in_control == 0 & n_participants_changed_in_intervention == 0))
  
  
  # run simulation ----
  simulation_condition_changing <- 
    # using the experiment parameters
    experiment_parameters_grid |>
  ...
  write_rds(simulation_condition_changing, "../data/simulation_condition_changing.rds", compress = "gz")
...
ggplot(simulation_condition_changing$altered_data[[iteration]], aes(condition, score, color = altered)) +
  geom_point(position = position_jitter(width = 0.1)) +
  scale_color_viridis_d(begin = 0.3, end = 0.7) +
  theme_linedraw()

bind_cols(
  simulation_condition_changing$analysis_results_original_data[[iteration]] |>
    rename(p_original = p),
  simulation_condition_changing$analysis_results_altered_data[[iteration]] |>
    rename(p_altered = p),
) |>
  mutate_if(is.numeric, janitor::round_half_up, digits = 3) |>
  kable() |>
...
# summarise simulation results over the iterations ----
simulation_condition_changing_summary <- simulation_condition_changing |>
  # convert `analysis_results` nested-data-frame column to regular columns in the df. in this case, the p value.
  unnest(analysis_results_original_data) |>
  rename(p_original_data = p) |>
  unnest(analysis_results_altered_data) |>
  rename(p_altered_data = p) |>
  # summarize across iterations
  group_by(n_per_condition, n_participants_changed_in_control, n_participants_changed_in_intervention) |>
  summarise(original_data_false_positive_rate = mean(p_original_data < .05),
            altered_data_false_positive_rate = mean(p_altered_data < .05)) |>
  ungroup()

simulation_condition_changing_summary |>
  mutate_if(is.numeric, janitor::round_half_up, digits = 2) |>
  kable() |>
  kable_classic(full_width = FALSE)
...
data_summary_ordered <- simulation_condition_changing_summary |>
  # the outcome variable must be called `outcome`
  # eg the estimand of your simulation, whether the false positive rate, estimated effect size, etc.
  # replace `positive_rate` with
  mutate(outcome = altered_data_false_positive_rate) |>
  ...
  # order the rows within the columns in the way you want them to appear left to right
  arrange(desc(n_per_condition), n_participants_changed_in_control, n_participants_changed_in_intervention) |>
  mutate(rank = row_number()) |>
  ...
  # select only the columns that you had multiple values for in the expand.grid() call in the simulation as well as `outcome` and `rank`
  # rename them if necessary to plot nicely
  mutate(n_participants_changed_in_control = as.factor(n_participants_changed_in_control),
         n_participants_changed_in_intervention = as.factor(n_participants_changed_in_intervention),
         n_per_condition = as.factor(n_per_condition)) |>
  dplyr::select(`N from control changed` = n_participants_changed_in_control, 
                `N from intervention changed` = n_participants_changed_in_intervention, 
                `N participants` = n_per_condition,
                outcome,
                rank) 
...
ggsave(filename = "../communications/plots/changed.pdf",
       plot = p_multiverse,
       device = "pdf",
       width = 6,
       height = 5)
